...
add_action( 'init', array( 'WS_Nav_Menu_Item_Custom_Fields', 'setup' ) );

class WS_Nav_Menu_Item_Custom_Fields {
            ...
            <p class="additional-menu-field-{name} description description-thin">
                <label for="edit-menu-item-{name}-{id}">
                    ...
                    <input
                        ...
                        class="widefat icon-picker code edit-menu-item-{name}"
	...
	static function wp_my_nav(){
        return 'WS_Walker_Nav_Menu_Edit';
	}
    ...
    static function setup() {
        ...
        $new_fields = apply_filters( 'ws_nav_menu_item_additional_fields', array() );
        if ( empty($new_fields) )
            return;
        self::$options['fields'] = self::get_fields_schema( $new_fields );

        add_filter( 'wp_edit_nav_menu_walker', 'WS_Walker_Nav_Menu_Edit');
        //add_filter( 'ws_nav_menu_item_additional_fields', array( __CLASS__, '_add_fields' ), 10, 5 );
     ...
     */
    static function get_field( $item, $depth=0, $args=array (), $id = 0 ) {
        $new_fields = '';
...
class WS_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
    function start_el(&$output, $item, $depth=0, $args=array (), $id = 0) {
        $item_output = '';
        parent::start_el($item_output, $item, $depth, $args);
        // Inject $new_fields before: <div class="menu-item-actions description-wide submitbox">
        if ( $new_fields = WS_Nav_Menu_Item_Custom_Fields::get_field( $item, $depth, $args ) ) {
