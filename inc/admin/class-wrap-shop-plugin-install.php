...
if ( ! class_exists( 'WrapShop_Plugin_Install' ) ) :
	/**
	 * The WrapShop plugin install class
	 */
	class WrapShop_Plugin_Install {
		...
		public function plugin_install_scripts( $hook_suffix ) {
			...
			wp_enqueue_script( 'wrapshop-plugin-install', get_template_directory_uri() . '/assets/js/admin/plugin-install' . $suffix . '.js', array( 'jquery', 'updates' ), $wrapshop_version, 'all' );

			wp_enqueue_style( 'wrapshop-plugin-install', get_template_directory_uri() . '/assets/sass/admin/plugin-install.css', array(), $wrapshop_version, 'all' );
		}
		 ...
		 */
		public static function install_plugin_button( $plugin_slug, $plugin_file, $plugin_name, $classes = array(),$required = 0, $activated = '', $activate = '', $install = '') {
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						...
						'url'     => '#',
						'classes' => array( 'wrapshop-button', 'disabled' ),
					);
					...
					if($required==1){
						$button = array(
							...
							'url'     => $url,
							'classes' => array( 'wrapshop-button active-now', 'activate-now' ),
						);
					}else{
						$button = array(
							'message' => esc_attr__( 'Activate', 'wrapshop' ),
							'url'     => $url,
							'classes' => array( 'wrapshop-button', 'activate-now' ),
						);
					}
					...
					if($required==1){
						$button = array(
							...
							'url'     => $url,
							'classes' => array( 'wrapshop-button required', 'sf-install-now', 'install-now', 'install-' . $plugin_slug ),
						);
					}else{
						$button = array(
							'message' => esc_attr__( 'Install now', 'wrapshop' ),
							'url'     => $url,
							'classes' => array( 'wrapshop-button', 'sf-install-now', 'install-now', 'install-' . $plugin_slug ),
						);
					}
...
return new WrapShop_Plugin_Install();